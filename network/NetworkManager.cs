...
public partial class NetworkManager : Node {
    ...
    private Dictionary<IPEndPoint, Guid> _endPointIds = new();
    ...
    public void Setup(NetworkServerType serverType) {
        ...
        foreach((Type typ, string eventName) in models) {
            models[typ] = eventName.PadRight(_buffer.Length, ' ');
        }
        var keys = invokers.Keys.ToArray();
        foreach(string key in keys) {
            invokers[key.PadRight(_buffer.Length, ' ')] = invokers[key];
            invokers.Remove(key);
        }
    ...
    internal void QueueMessage(
        ...
        var eventNameBytes = Encoding.UTF8.GetBytes(_models[model.GetType()].PadRight(_buffer.Length, ' '));
    ...
    public void SendToAll(NetworkEventModel netEvent, bool isRel) {
        QueueMessage(DatagramOutgoingSendType.SendTo, netEvent, isRel, _endPointIds.Keys.ToArray());
    }
    public void SendToOthers(IPEndPoint allButEndPoint, NetworkEventModel netEvent, bool isRel) {
        QueueMessage(DatagramOutgoingSendType.SendTo, netEvent, isRel, _endPointIds.Keys.Where(p => p != allButEndPoint).ToArray());
    }
    ...
    private void OnMessageReceived(object? _sender, DatagramIncomingMessage callback) {
        try {
            var eventName = Encoding.UTF8.GetString(callback.Data, 0, _buffer.Length).PadRight(_buffer.Length, ' ');
    ...
    private NetworkAuthResult HandleAuth(DatagramIncomingMessage callback, string eventName) {
        var bytes = callback.Data[_buffer.Length..];
