...
{	
	...
	[Export] private UiCanvas _uiCanvas = default!;
				...
				if(_draggingTool.IsDragging) {
					...
					foreach(var token in _selectedTokens) {
						if(_netManager.IsHost) {
							_netManager.SendToAll(new TokenMovedModel(token.Id, token.MapPosition.X, token.MapPosition.Y), true);
						} else {
							_netManager.SendToHost(new TokenMovedModel(token.Id, token.MapPosition.X, token.MapPosition.Y), true);
						}
					}
				}
